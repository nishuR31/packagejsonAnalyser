...
export let login = asyncHandler(async (req, res) => {
  ...
  if (req.user || exist) {
    return res.status(codes.ok).json(
      new ApiResponse(
        "User already logged in,try loggin out to login again.",
        codes.ok,
        {
          user: {
            _id: req.user?._id ?? exist._id,
            userName: req.user?.userName ?? exist.userName,
            role: req.user?.role ?? exsit.role,
          },
        }
      ).res()
    );
...
export let logout = asyncHandler(async (req, res) => {
  ...
  await red.del(...(await red.keys(`user:*`)));

  return res
    .status(codes.ok)
    .json(new ApiResponse("You are successfully logged out.", codes.ok).res());
});
...
export let getAllUsers = asyncHandler(async (req, res) => {
  let exist = JSON.parse(await red.hGet(`user:${process.env.KEY}`, "allUsers"));
  if (exist) {
    return res.status(codes.ok).json(
  ...
  await red.hSet(`user:${process.env.KEY}`, "allUsers", JSON.stringify(users));
  return res.status(codes.ok).json(
    new ApiResponse("Users successfully found", codes.ok, {
